...
And(/^I fill in my refund original case details$/) do
  ...
  step('I take a screenshot named "Page 3 - Original Case Details 6 "')
...
And(/^I fill in all my refund fees but do not select a payment method$/) do
  refund_original_case_details_page.original_claim_fees.et_issue.fee.set(1)
  refund_original_case_details_page.original_claim_fees.et_hearing.fee.set(1)
  refund_original_case_details_page.original_claim_fees.et_reconsideration.fee.set(1)
  refund_original_case_details_page.original_claim_fees.eat_issue.fee.set(1)
  refund_original_case_details_page.original_claim_fees.eat_hearing.fee.set(1)
end


Then(/^all fee payment method fields in the refund case details should be marked with an error$/) do
  expect(refund_original_case_details_page.original_claim_fees.et_issue.payment_method.error.text).to eql 'Please select a payment method'
  expect(refund_original_case_details_page.original_claim_fees.et_hearing.payment_method.error.text).to eql 'Please select a payment method'
  expect(refund_original_case_details_page.original_claim_fees.et_reconsideration.payment_method.error.text).to eql 'Please select a payment method'
  expect(refund_original_case_details_page.original_claim_fees.eat_issue.payment_method.error.text).to eql 'Please select a payment method'
  expect(refund_original_case_details_page.original_claim_fees.eat_hearing.payment_method.error.text).to eql 'Please select a payment method'
end


And(/^the fee fields in the refunds case details should not be marked with any errors$/) do
  aggregate_failures do
    expect(refund_original_case_details_page.original_claim_fees.et_issue.payment_method).to have_no_error, 'Expected et issue payment method not to have an error'
    expect(refund_original_case_details_page.original_claim_fees.et_hearing.payment_method).to have_no_error, 'Expected et hearing payment method not to have an error'
    expect(refund_original_case_details_page.original_claim_fees.et_reconsideration.payment_method).to have_no_error, 'Expected et reconsideration payment method not to have an error'
    expect(refund_original_case_details_page.original_claim_fees.eat_issue.payment_method).to have_no_error, 'Expected eat issue payment method not to have an error'
    expect(refund_original_case_details_page.original_claim_fees.eat_hearing.payment_method).to have_no_error, 'Expected eat hearing payment method not to have an error'
    expect(refund_original_case_details_page.original_claim_fees.et_issue.fee).to have_no_error, 'Expected et issue fee not to have an error'
    expect(refund_original_case_details_page.original_claim_fees.et_hearing.fee).to have_no_error, 'Expected et hearing fee not to have an error'
    expect(refund_original_case_details_page.original_claim_fees.et_reconsideration.fee).to have_no_error, 'Expected et reconsideration fee not to have an error'
    expect(refund_original_case_details_page.original_claim_fees.eat_issue.fee).to have_no_error, 'Expected eat issue fee not to have an error'
    expect(refund_original_case_details_page.original_claim_fees.eat_hearing.fee).to have_no_error, 'Expected eat hearing fee not to have an error'
  end
end
