...
namespace :storage do
  desc 'Updates any non submitted claims which have group claims csv in azure'
  task update_group_claims_csv: :environment do
    ...
    scope.all.each do |claim|
      ...
      if request.response.success?
        ...
        puts "Error - response code #{request.response_code}"
        errors << claim.id
      end
    end
  ...
  desc 'Updates any non submitted claims which have claim details rtf in azure'
  task update_claim_details_rtf: :environment do
    build_blob_service_class = Class.new(::ApiService) do
      def submit
        json = {
          uuid: SecureRandom.uuid,
    ...
    upgraded = []
    errors = []
    scope = Claim.where.not(state: :submitted).where.not(claim_details_rtf: nil).where("claim_details_rtf NOT LIKE '---%'")
    puts "#{scope.count} to be upgraded"
    credentials = {
      provider:              'AWS',
      region:                ENV.fetch('AWS_REGION', 'eu-west-1'),
    ...
    scope.all.each do |claim|
      filename = claim.attributes['claim_details_rtf']
      key = "uploads/claim/claim_details_rtf/#{claim.reference}/#{filename}"
      file = bucket.files.get(key)
      next if file.nil?
      ...
      if request.response.success?
        claim.claim_details_rtf = { filename: filename, content_type: file.content_type, path: file_key }.with_indifferent_access
        claim.save validate: false, touch: false
        upgraded << claim
      else
        puts "Error - response code #{request.response.code}"
        errors << claim.id
      end
    end
