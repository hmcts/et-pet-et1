...
RSpec.describe Claim, :type => :claim do
  ...
  describe '#reference' do
    it 'returns a token generated by ApplicationReference' do
      expect(ApplicationReference).to receive(:generate) { 'ABCD1234' }
      expect(subject.reference).to eq('ABCD1234')
    end
  end

  describe '.find_by_reference' do
    it 'returns the claim with the corresponding reference' do
      claim = described_class.create!(application_reference: 'ABCD1234')
      expect(described_class.find_by_reference('ABCD1234')).to eql(claim)
    end

    it 'normalises the reference before looking up' do
      claim = described_class.create!(application_reference: 'ABCD1234')
      expect(described_class.find_by_reference('abcdl234')).to eql(claim)
    end
  end
